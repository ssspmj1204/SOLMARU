...
//	      gbc_lbl_back.ipadx = 15;
		...
		// 장바구니db에 있는 값중 주문db에 있는 값을 제외하고 테이블에 추가 끝
		...
		JLabel lblNewLabel_1 = new JLabel("샐러디");
		...
		StoreVO svo = sdao.select("salady");
		...
		foodlist = food_dao.selectStorefood("salady");
			...
			public void mouseClicked(MouseEvent e) {// 결제완료창으로 가기
				if (table.getRowCount() == 0) {
					...
					if (result == JOptionPane.CLOSED_OPTION) {} // 그냥 껐음
						...
						if(n_bvo.size() != 0 ) {
							odao.insert(n_bvo, "salady");
						}
						StoreDAO sdao = new StoreDAO();
						sdao.update_cnt_plus("salady");

...
//	      gbc_lbl_back.ipadx = 15;
		...
		JLabel lblNewLabel_1 = new JLabel("제라진돈까스");
		...
		StoreVO svo = sdao.select("제라진돈까스");
		...
		foodlist = food_dao.selectStorefood("제라진돈까스");
			...
			public void mouseClicked(MouseEvent e) {// 결제완료창으로 가기
				if (table.getRowCount() == 0) {
					...
					if (result == JOptionPane.CLOSED_OPTION) {
						...
						if(n_bvo.size() != 0 ) {
							odao.insert(n_bvo, "제라진돈까스");
						}
						StoreDAO sdao = new StoreDAO();
						sdao.update_cnt_plus("제라진돈까스");

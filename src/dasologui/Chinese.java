...
//	      gbc_lbl_back.ipadx = 15;
		...
		JLabel lblNewLabel_1 = new JLabel("중화가정");
		...
		foodlist = food_dao.selectStorefood("중화가정");
		...
		lblNewLabel_40.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {// 결제완료창으로 가기
				if (table.getRowCount() == 0) {
					...
					int result = JOptionPane.showConfirmDialog(frame, "이대로 주문 하시겠습니까?","마지막 선택",JOptionPane.YES_NO_OPTION);
					if(result == JOptionPane.CLOSED_OPTION) {}//그냥 껐음
						...
						if(n_bvo.size() != 0 ) {
							odao.insert(n_bvo, "중화가정");
						}
						StoreDAO sdao = new StoreDAO();
						sdao.update_cnt_plus("중화가정");
